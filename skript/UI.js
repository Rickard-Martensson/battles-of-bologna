...
class UIHandler {
    ...
    drawImage(Image, dx, dy, sx, sy, ctxX, ctxY, ctxSx, ctxSy, shouldRound = true) {
        ...
        if (shouldRound) {
            for (let i = 0; i < ctxList.length; i++) {
                ctxList[i] = Math.floor(ctxList[i]);
            }
        }
    ...
    drawBox(pos1, pos2, fillColor, borderColor, alpha = 1, borderalpha = 0) {
        ...
        ctx.fillRect(pos1.x * S, pos1.y * S, (pos2.x - pos1.x) * S, (pos2.y - pos1.y) * S);
        if (borderalpha != 0) {
            ctx.strokeStyle = borderColor;
            ctx.globalAlpha = borderalpha;
            ctx.lineWidth = S;
            ctx.lineJoin = 'miter';
            ctx.strokeRect(pos1.x * S, pos1.y * S, (pos2.x - pos1.x) * S, (pos2.y - pos1.y) * S);
        }
        ctx.globalAlpha = 1;
    }
    ...
    drawInfoBox(team) {
        ...
        let playerClan = infoPlayer.getClan();
        let button = BTN_FOLDER[playerClan][infoPlayer.currentFolder][this.currentButton.id];
        let x_pos = 110;
        let y_pos = 150;
        if (infoPlayer.currentFolder == 3 && this.currentButton.id == 2 && infoPlayer.btnLvl < 4) {
            button = BTN_FOLDER[playerClan][infoPlayer.currentFolder][6];
        }
        if (button != undefined && (this.currentButton.team == 0 || this.isOnline) && this.btnIsEnabled(button.action, infoPlayer, button.upgrade, button.lvl) && "info" in button) {
            ...
            ctx.fillText(title, (132) * S, (145) * S);
            ...
            for (var i in info) {
                ctx.fillText(info[i], (132) * S, (151 + 5 * parseInt(i)) * S);
            }
    ...
    drawChatBox2(team) {
        ...
        for (var i in this.chats) {
            ...
            ctx.fillText(sender + ": " + msg, (msgPos) * S, (140 + 5 * parseInt(i)) * S);
            // ctx.font = 4 * S + "px 'iFlash 705'";
            // ctx.fillText(msg,
            //     (msgPos + 30) * S,
            //     (140 + 5 * i) * S,
        ...
        ctx.fillText(myName + ": " + displayMsg + typingBlink, (namePos - 1) * S, (143 + 5 * 6) * S);
    }
    drawEverything(fps) {
        ...
        if (DAY_NIGHT_ENABLED) {
            this.changeBackground(Date.now() - game.startTime);
        }
        ;
        if (CLOUDS_ENABLED && this.sceneryCount < CLOUD_MAX_COUNT) {
            this.tryMakeCloud();
        }
        ;
        this.drawUI(fps);
    ...
    buttonAction(id, team) {
        ...
        let btnGlob = BTN_FOLDER[playerClan][curFolder][id];
        if (curFolder == 3 && id == 2 && player.btnLvl != ABILITY_MAX_LVL) {
            btnGlob = BTN_FOLDER[playerClan][curFolder][6];
        }
    ...
    castAbility(abilityName, team, abilityCooldown, posX = 0) {
        if (this.isOnline) {
            //some pubnub shit
        ...
        for (var playerKey in game.players) {
            ...
            if (playerQueueLen >= 2 || (Date.now() - this.dateQueueShow[playerKey] < 0.5 * 1000)) {
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    this.drawSmallIcon(iconName, 0, x_pos + (Number(playerKey) * -2 + 1) * 4 * i, y_pos);
                }
            }
            ctx.imageSmoothingEnabled = true;
            if (this.justGaveGold[playerKey]) {
                ctx.fillStyle = "#FFFFFF";
            }
            else {
            ...
            ;
            ctx.fillText(Math.floor(player.gold), UI_POS[playerKey].gold.x * S, UI_POS[playerKey].gold.y * S);
            if (this.justGaveGold[playerKey]) {
                ctx.fillStyle = "#F2F2AA";
            }
            else {
            ...
            ctx.fillText('+' + Math.floor(player.goldPerTurn), UI_POS[playerKey].goldPerTurn.x * S, UI_POS[playerKey].goldPerTurn.y * S);
            let goldIconSize = 6;
            this.drawImage(Images["gold"], 0, 0, 16, 16, (UI_POS[playerKey].goldIcon.x) * S, (UI_POS[playerKey].goldIcon.y - goldIconSize / 2) * S, goldIconSize * S, goldIconSize * S);
            this.playParticles();
    ...
    drawHpBars(playerKey, player) {
        ...
        hpBarImgWidth * invertColors, hpBarImgHeight * 5, hpBarImgWidth, hpBarImgHeight, (UI_POS[playerKey].hpBar.x) * S, (UI_POS[playerKey].hpBar.y - hpBarHeight / 2 + hpBarJump) * S, hpBarWidth * S, hpBarHeight * S);
        this.drawImage(Images["hpBars"], //previous dmg
        hpBarImgWidth * invertColors, hpBarImgHeight * 4, hpBarImgWidth * prevDmgFrac, hpBarImgHeight, (UI_POS[playerKey].hpBar.x) * S, (UI_POS[playerKey].hpBar.y - hpBarHeight / 2 + hpBarJump) * S, hpBarWidth * S * prevDmgFrac, hpBarHeight * S);
        this.drawImage(Images["hpBars"], hpBarImgWidth * invertColors, hpBarImgHeight * hpBarColor, hpBarImgWidth * dmgFrac, hpBarImgHeight, (UI_POS[playerKey].hpBar.x) * S, (UI_POS[playerKey].hpBar.y - hpBarHeight / 2 + hpBarJump) * S, hpBarWidth * S * dmgFrac, hpBarHeight * S);
    }
    btnIsEnabled(action, player, upgRequired, lvl) {
        if (action == "hidden") {
            return false;
        }
        else if (Number(action != "upgrade") ^ player.checkIfResearched(upgRequired)) { //XOR, coolt. false ^ true
            return false;
        }
    ...
    writeBtnText(text, pos, name, yoffset = 0) {
        ctx.fillText(text, (pos.x + UI_POS_BTN[name].x) * S, (pos.y + UI_POS_BTN[name].y + yoffset) * S);
    }
    drawButtons(team) {
        for (const [index, item] of BTN_LAYOUT[team].entries()) {
            let player = this.getPlayer(team);
            let playerClan = player.getClan();
            let curFolder = player.currentFolder;
            let btn = BTN_FOLDER[playerClan][curFolder][index];
            ...
            if (this.btnIsEnabled(action, player, upgrade, lvl)) {
                ...
                btn = BTN_FOLDER[playerClan][curFolder][6];
                this.drawButton(index, item, btn, player, team);
            }
        }
    }
    drawButton(index, button, btnGlob, player, team) {
        ...
        ctx.imageSmoothingEnabled = false;
        this.drawImage(Images.button1, BTN_SIZE * frame, BTN_SIZE * 1, BTN_SIZE, BTN_SIZE, (button.x - BUTTON_SIZE / 2) * S, (button.y - BUTTON_SIZE / 2) * S, BUTTON_SIZE * S, BUTTON_SIZE * S, true);
        if (cooldownFrac != undefined) {
            ...
            // );
            this.drawImage(Images.button1, BTN_SIZE * 2, BTN_SIZE * 1 + BTN_SIZE * cooldownFrac, BTN_SIZE, BTN_SIZE * (1 - cooldownFrac), Math.floor((button.x - BUTTON_SIZE / 2) * S), (button.y - BUTTON_SIZE / 2 + BUTTON_SIZE * cooldownFrac) * S, Math.floor(BUTTON_SIZE * S), BUTTON_SIZE * S * (1 - cooldownFrac), false);
        }
        ...
        if (text2 !== undefined) {
            this.writeBtnText(text2, button, "txt2", frame);
        }
        if (cost == "%upggold%") {
            cost = player.goldPerTurn + UPGRADES["upgGold"].costDelta;
        }
        else if (cost == "%repaircastle%") {
            cost = player.repairCost;
        }
        else if (cost == "%upgcastle%") {
            cost = 50;
        }
        else if (cost == "%upgability%") {
            cost = 25 + player.btnLvl * 5;
        }
        ctx.textAlign = "end";
        ...
        if (btnIcon !== null) {
            let goldIconSize = 5;
            this.drawImage(Images["gold"], 16 * btnIcon, 16 * btnIcon, 16, 16, (button.x + UI_POS_BTN.gold.x) * S, (button.y + UI_POS_BTN.gold.y + frame - goldIconSize / 2) * S, goldIconSize * S, goldIconSize * S);
        }
    }
}
