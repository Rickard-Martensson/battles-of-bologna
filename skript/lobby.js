...
function updateMenu() {
    ...
    for (var i = 0; i < elementActive.length; i++) {
        elementActive[i].style.display = "block";
    }
}
function menuLocal() {
...
function menuJoinLobby() {
    ...
    joinLbyInputField.addEventListener("keyup", function (e) {
        if (e.key == "Enter") {
            ...
            document.getElementById("lobby_input").value = document.getElementById("lobby_input").value.toLocaleUpperCase();
...
function promptJoin() {
    thisLobbyCode = document.getElementById("lobby_input").value.toLocaleUpperCase();
    myName = document.getElementById("username_input").value;
    console.log(myName, "myName");
    console.log("trying to join:", thisLobbyCode, myName);
    pubnub.hereNow({
