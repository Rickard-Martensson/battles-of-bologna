...
class UIHandler {
    players: number[];
    isOnline: boolean;
    timeSinceGold: number;
    activeButtons: {}[];
    disabledButtons: {}[];
    mousePos: { x: number; y: number; };
    currentButton: { team: number; id: number; };
    sceneryCount: number;
    scenery: Scenery[];
    justGaveGold: any[];
    chats: any[];
    ...
    winner: number;
    curMsg: string;
    lastTypingBlink: number;
    isTyping: boolean;
    isHurry: boolean;
    isTouch: boolean;
    activeParticles: any[];
    dateQueueShow: { 0: number; 1: number; };
    chatmsgs: any;
    constructor(players: number[], isOnline) {
    ...
    drawImage(Image: HTMLImageElement, dx: number, dy: number, sx: number, sy: number, ctxX: number, ctxY: number, ctxSx: number, ctxSy: number, shouldRound = true) {
    ...
    buttonAction(id, team) {
        ...
        if (curFolder == 3 && id == 2 && player.btnLvl != ABILITY_MAX_LVL) { btnGlob = BTN_FOLDER[playerClan][curFolder][6] }
    ...
    drawButtons(team) {
        for (const [index, item] of BTN_LAYOUT[team].entries()) {
            let player = this.getPlayer(team);
            let playerClan: ClanTypes = player.getClan()
            let curFolder = player.currentFolder;
